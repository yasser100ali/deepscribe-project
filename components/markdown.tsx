  ...
  const components: Partial<Components> = {
    ...
    code: ({ node, inline, className, children, ...props }) => {
      ...
      return !inline && match ? (
        ...
        <pre
          {...props}
          className={`${className} text-sm w-[80dvw] md:max-w-[500px] overflow-x-scroll bg-muted/60 border border-border/40 p-4 rounded-xl mt-2`}
        >
          <code className={match[1]}>{children}</code>
        </pre>
      ) : (
        <code
          className={`${className} text-sm bg-muted/60 border border-border/40 py-0.5 px-1.5 rounded-md font-mono text-primary`}
          {...props}
        >
          {children}
        </code>
